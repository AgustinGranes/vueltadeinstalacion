...
const CATEGORY_LOCAL_LOGOS: Record<string, string> = {
  ...
  'NASCAR Trucks': '/categories/nascartruck.png',
...
function _deduplicateRaces(primaryRaces: Race[], secondaryRaces: Race[]): Race[] {
  ...
  for (const key of allKeys) {
    ...
    if (primRaces.length > 0 && secRaces.length === 0) {
      ...
      const overlaps = primScheds.some(p => {
        const pT = (p as any).startAt || (p as any).start;
        return secScheds.some(s => {
          const sT = (s as any).startAt || (s as any).start;
