...
export const dataService = {
  ...
  async getWRCCalendar(): Promise<WRCCalendarEvent[]> {
    ...
    try {
      // Fetch both current/upcoming and past events from the live site
      ...
      const parseHtml = (html: string, isPast: boolean) => {
        const doc = new DOMParser().parseFromString(html, 'text/html');
        // Look for event cards or table rows - WRC uses several layouts
        const cards = doc.querySelectorAll('.event-feed-card, .event-list__row, .rally-card, article[class*="event"]');
        
        cards.forEach((el) => {
          const titleEl = el.querySelector('.event-feed-card__title, .event-list__rally-name, [class*="title"], h3, h2');
          const title = titleEl?.textContent?.trim() || '';
          if (!title || !title.toLowerCase().includes('rally')) return;

          const rallyName = title.replace(/^WRC\s+/i, '').replace(/\s+\d{4}$/, '');
          const dates = el.querySelector('.event-feed-card__date-text, .event-list__date, .date, [class*="date"]')?.textContent?.trim() || '';
          ...
          const parseWRCDate = (dateStr: string): { start: Date; end: Date } | null => {
            try {
              ...
              // Support all dash types: hyphen (-), n-dash (–), m-dash (—)
              const cleanStr = dateStr.toLowerCase().replace(/\s+/g, ' ').replace(/[\u2013\u2014]/g, '-');
              let startDay = 0, startMonth = -1, endDay = 0, endMonth = -1, year = new Date().getFullYear();

              // Format: "12 - 15 march 2026"
              const simpleMatch = cleanStr.match(/(\d+)\s*-\s*(\d+)\s+([a-z]{3,})\s*(\d{4})?/);
              if (simpleMatch) {
                startDay = parseInt(simpleMatch[1]);
                endDay = parseInt(simpleMatch[2]);
                const mFull = simpleMatch[3].substring(0, 3);
                startMonth = endMonth = months[mFull] ?? -1;
                if (simpleMatch[4]) year = parseInt(simpleMatch[4]);
              } else {
                // Format: "30 jan - 02 feb 2026"
                const crossMonthMatch = cleanStr.match(/(\d+)\s+([a-z]{3,})\s*-\s*(\d+)\s+([a-z]{3,})\s*(\d{4})?/);
                if (crossMonthMatch) {
                  ...
                  if (crossMonthMatch[5]) year = parseInt(crossMonthMatch[5]);
                }
              }
          ...
          // Determine status - Prioritize explicit text indicators first
          let status: WRCCalendarEvent['status'] = isPast ? 'Finished' : 'Upcoming';
          const cardText = el.textContent?.toLowerCase() || '';
          
          if (cardText.includes('happening now') || cardText.includes('live')) {
            status = 'Live';
          } else if (cardText.includes('upcoming event')) {
            status = 'Upcoming';
          } else {
          ...
          const roundMatch = title.match(/Round\s+(\d+)/i) || textContent.match(/Round\s+(\d+)/i);
          if (roundMatch) round = parseInt(roundMatch[1]);
          
          // Try to get round from a specific badge or span if common classes exist
          if (!round) {
            const badge = el.querySelector('.badge, .round, .event-feed-card__round')?.textContent;
            const badgeMatch = badge?.match(/(\d+)/);
            if (badgeMatch) round = parseInt(badgeMatch[1]);
          }

          // Avoid duplicates
          if (!events.some(e => e.rallyName === rallyName)) {
      ...
      // Sort: if we have rounds, use them. Otherwise, past events first.
      // But actually, we want them in chronological order.
      // Since past events are already chronological in the 'past' page,
      // and upcoming are too, merging them in this order works well.
      // We'll just ensure sequential rounds 1, 2, 3...
      events.forEach((ev, idx) => {
        if (!ev.round) ev.round = idx + 1;
      });

      // Special case: if we found some rounds (say 5, 6) but others are 0,
      // sorting might be weird. Let's just trust our sequential assignment if there's inconsistency.
      // Better: Sort by round if any > 0, otherwise stay in discovered order.
      const hasRealRounds = events.some(e => e.round > 0);
      if (hasRealRounds) {
        events.sort((a, b) => (a.round || 99) - (b.round || 99));
        // Re-assign sequentially to ensure no gaps (e.g. 1, 2, 5 -> 1, 2, 3)
      ...
      console.error('[DataService] WRC official calendar error:', e);
      return [];
    }
  },
